...
import { Building2, CalendarDays, CheckCircle, Clock, MapPin, Stethoscope } from "lucide-react";
import { currentStepLabel, isMedicalBookingRequired, STATUS_COLORS, statusLabel } from "./application-utils";
...
export default function BookMedical({ params }: { params: { id: string } }) {
  ...
  const centersParams = useMemo(() => ({ centerType: "HEALTH_CENTER", governorate: governorate || undefined }), [governorate]);
  ...
  const { data: slots, isLoading: slotsLoading } = useQuery({
    queryKey: ["appointment-slots", centerId, date, "MEDICAL_TEST"],
    enabled: !!centerId && !!date,
    queryFn: async () => {
      const response = await fetch(`/api/appointments/slots?centerId=${encodeURIComponent(centerId)}&date=${encodeURIComponent(date)}&type=MEDICAL_TEST`, {
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      toast({
        title: language === "ar" ? "تم حجز موعد فحص النظر بنجاح." : "Medical/vision test appointment booked successfully.",
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["appointment-slots", centerId, date, "MEDICAL_TEST"] }),
      ]);
  ...
  const canBook = isMedicalBookingRequired(detail);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">{language === "ar" ? "حجز فحص النظر" : "Book Medical / Vision Test"}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {language === "ar" ? "اختر مركزاً صحياً حكومياً وموعداً متاحاً لاستكمال طلبك." : "Choose a government health center and an available time slot to continue your application."}
          </p>
        </div>
      </div>
      ...
      {!canBook && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="p-4 text-sm text-amber-900">
            {language === "ar" ? "هذا الطلب ليس في مرحلة حجز فحص النظر حالياً." : "This application is not currently ready for medical/vision test booking."}
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">{language === "ar" ? "المركز الصحي الحكومي" : "Government Health Center"}</label>
            {centersLoading ? <Skeleton className="h-24" /> : centers && centers.length > 0 ? (
              ...
              <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
                {language === "ar" ? "لا توجد مراكز صحية متاحة في هذه المحافظة." : "No government health centers are available in this governorate."}
              </div>
            )}
          </div>
