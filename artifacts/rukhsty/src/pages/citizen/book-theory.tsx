...
import { Building2, CalendarDays, CheckCircle, Clock, MapPin, BookOpen } from "lucide-react";
import { currentStepLabel, examRebookingInfo, isTheoryBookingRequired, STATUS_COLORS, statusLabel } from "./application-utils";
...
export default function BookTheory({ params }: { params: { id: string } }) {
  ...
  const minimumDate = rebooking?.earliestDate && rebooking.earliestDate > todayIso() ? rebooking.earliestDate : todayIso();
  ...
  const centersParams = useMemo(() => ({ centerType: "EXAM_CENTER", governorate: governorate || undefined }), [governorate]);
  ...
  const { data: slots, isLoading: slotsLoading } = useQuery({
    queryKey: ["appointment-slots", centerId, date, "THEORY_EXAM"],
    enabled: !!centerId && !!date,
    queryFn: async () => {
      const response = await fetch(`/api/appointments/slots?centerId=${encodeURIComponent(centerId)}&date=${encodeURIComponent(date)}&type=THEORY_EXAM`, {
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      toast({
        title: language === "ar" ? "تم حجز موعد الامتحان النظري بنجاح." : "Theory exam appointment booked successfully.",
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["appointment-slots", centerId, date, "THEORY_EXAM"] }),
      ]);
  ...
  const canBook = isTheoryBookingRequired(detail);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">{language === "ar" ? "حجز الامتحان النظري" : "Book Theory Exam"}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {language === "ar" ? "اختر مركز امتحان معتمداً وموعداً متاحاً لاستكمال طلبك." : "Choose an authorized exam center and an available time slot to continue your application."}
          </p>
        </div>
      </div>
      ...
      {!canBook && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="p-4 text-sm text-amber-900">
            {language === "ar" ? "هذا الطلب ليس في مرحلة حجز الامتحان النظري حالياً." : "This application is not currently ready for theory exam booking."}
          </CardContent>
        </Card>
      )}
      ...
      {rebooking && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-4 text-sm text-red-900">
            <p className="font-semibold">{language === "ar" ? "لم يتم اجتياز الامتحان النظري" : "Theory exam was not passed"}</p>
            <p className="mt-1">
              {language === "ar"
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">{language === "ar" ? "مركز الامتحان النظري" : "Theory Exam Center"}</label>
            {centersLoading ? <Skeleton className="h-24" /> : centers && centers.length > 0 ? (
              ...
              <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
                {language === "ar" ? "لا توجد مراكز امتحان متاحة في هذه المحافظة." : "No theory exam centers are available in this governorate."}
              </div>
            )}
          </div>
